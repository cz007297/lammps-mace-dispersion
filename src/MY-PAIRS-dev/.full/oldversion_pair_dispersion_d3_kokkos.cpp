...
{
  // Make sure we get the *device* views in a dependent context
  ...
  // Grab device‑accessible neighbor list data
  auto d_inum        = list->inum  ;
  auto d_ilist       = view_1i("ilist", nmax) ;
  auto d_numneigh    = view_1i("numneigh", nmax) ;
  auto d_firstneigh  = view_2i("firstnight", nmax);
  ...
  Kokkos::parallel_for(
    "CalcCN",
    Kokkos::RangePolicy<DeviceType>(0, d_inum),
    KOKKOS_LAMBDA (const int ii)
  {
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      const double rcov_ij = (d_rcov(itype) + d_rcov(jtype)) * l_autoang;
      const double cn_ij   = 1.0 / (1.0 + exp(-l_K1 * ((rcov_ij / rr) - 1.0)));
...
template<class DeviceType>
void PairDispersionD3Kokkos<DeviceType>::k_compute_dC6_pair(
...
{
  DerivativeC6Functor<DeviceType> functor{
    iat, jat, cni, cnj,
    d_c6ab, k_mxci,
  ...
  // single‑iteration parallel_for just to run device code
  Kokkos::parallel_for("Get_dC6", Kokkos::RangePolicy<DeviceType>(0, 1), functor);
}
...
void get_dC6( int iat, int jat, double cni, double cnj, 
              const view_5d d_c6ab, const view_1d k_mxci,
              double l_autoev, double l_autoang, double l_K3, 
              double *res)
{
  double c6mem = -1.0e20;
...
template<class DeviceType>
double PairDispersionD3Kokkos<DeviceType>::k_compute_dC6(int iat, int jat, double cni, double cnj) 
{
  auto d_c6ab = this->c6ab_view; 
  auto d_mxci  = this->mxci_view; 
  auto d_cn = this->k_cn.template view<DeviceType>();
